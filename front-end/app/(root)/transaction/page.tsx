...
export default function Transaction() {
  ...
  const [validSelectedImage, setValidSelectedImage] = useState<File | null>(
    null
  );
  ...
  const [formPopulatedFromImage, setFormPopulatedFromImage] =
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const endpoint = `${process.env.NEXT_PUBLIC_API_URL}/${
        isEdit ? `transactions/${storedTransaction.id}` : "accounts/insert"
      }`;
      ...
      if (!res.ok) {
        ...
        console.error(
          ...
          await res.text().catch(() => "Could not read response text")
        );
        throw new Error(
          errorData.message ||
  ...
  const generateDescription = async (): Promise<void> => {
    ...
    try {
      ...
      let parsedData;
      try {
        const jsonString = data.description
          .toString()
          .replace("```json", "")
          .replace("```", "")
          .trim();
      ...
      const numericAmount = Math.abs(
        Number.parseFloat(parsedData.amount || "0")
      );
      ...
      const type =
        parsedData.type.toUpperCase() === "EXPENSE" ? "Expense" : "Income";
      setTransactionType(type);
      setPreviousType(type);
      ...
      <div className="w-full p-0 bg-white relative z-0">
        <div className="mx-auto max-w-md px-6 py-12 bg-white border-0 rounded-2xl sm:rounded-3xl">
          <h1 className="text-2xl mt-5">
            {isEdit ? "Edit record" : "How much do you spend today?"}
          </h1>
          ...
          <form id="form" onSubmit={handleSubmit}>
            ...
            <div className=" w-auto p-6 my-4 border-2 border-dashed rounded-lg  flex flex-col items-center">
              <legend className="description-small flex justify-between text-black m-2 text-center font-bold">
                <Icon icon="lucide:image" width="24" height="24" />
                Upload your record without filling information
              </legend>
              <div className="w-full m-2">{renderImageUploadModal()}</div>
            </div>
