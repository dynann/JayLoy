...
function page() {
  ...
  const handleLogout = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/auth/handleLogout`,
        {
          method: "PATCH",
          headers: {
      ...
      if (res.ok) {
        localStorage.removeItem("refreshToken"); //  remove refreshToken in  client side
        console.log("handleLogout successfully");
        router.push("/login");
      } else {
  ...
  const lists = [
    {
      title: "Edit Profile",
      icon: <Icon icon="iconamoon:profile-fill" width="24" height="24" />,
    },
    { title: "My Data", icon: <Icon icon="bxs:data" width="24" height="24" /> },
    {
      title: "Setting",
      icon: <Icon icon="lets-icons:setting-fill" width="24" height="24" />,
    },
    {
      title: "Terms and Conditions",
      icon: <Icon icon="famicons:document-text" width="24" height="24" />,
    },
    {
      title: "Log out",
      icon: <Icon icon="famicons:document-text" width="24" height="24" />,
    ...
    <div className={containerClasses}>
      ...
      <div className="space-y-2 flex flex-col items-center px-4">
        <Image
          ...
          alt="Neil image"
        />
        <p className="description-regular text-center">
          {username || "Loading..."}
        </p>
        <p className="description-regular text-center">
          {email || "Loading..."}
          ...
          {lists.map((item, index) => (
            <ul className=" ">
              <li key={index} className="m-2 sm:pb-1 ">
                <div
                  className="flex  items-center w-full space-x-4 rtl:space-x-reverse"
                  onClick={item.title === "Log out" ? handleLogout : undefined}
                >
                  {/* icon  */}
                  <div className="flex flex-col items-center justify-center py-2 text-gray">
                    <div
                      className={`bg-tertiary p-2 rounded-lg text-primary shadow-sm`}
                    >
                      {item.icon}
                    </div>
                  </div>
                  <div className="flex-1  min-w-0">
                    <p
                      className=" description-regular text-left text-black truncate dark:text-white"
                      // onClick= {item.title === "Log out"? handleLogout : undefined}
                    >
                      {item.title}{" "}
                    </p>
